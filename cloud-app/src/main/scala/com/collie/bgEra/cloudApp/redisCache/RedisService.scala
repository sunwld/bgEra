...
class RedisService {
  ...
  def hsetGetAllNoVlueDeser(key: String) : ju.Map[String, Array[Byte]] ={
    val valueMap: ju.Map[Array[Byte], Array[Byte]] = jedis.hgetAll(strSerializer.serialize(key))
    if(valueMap != null){
      valueMap.map(i => {
        (strSerializer.deserialize(i._1),i._2)
      })
    }else{
      null
    }
